...
def change_int_to_money_str(amount_int):
    return format(amount_int, ',')
def chagne_str_to_money_str(amount_str):
    pass
def change_int_to_KRW_str(amount_int):
    """ 입력된 원화를 4자리단위 한글로 변환한다 """
    str_result = "" # 결과문자열 초기화
    str_sign   = "" # 부호 초기화
    num_change = amount_int # 최초값을 모두 잔돈에 넣는다
